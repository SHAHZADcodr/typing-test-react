...
const Game = () => {
  ...
  const endGameCalled = useRef(false); // ✅ Ensure endGame is only called once
    ...
    const falling = setInterval(() => {
      setWords((prevWords) => {
        ...
        if (missedWords.length > 0) {
          setMissed((m) => m + missedWords.length); // ✅ Only add missed once per word
        }
  ...
  const handleTyping = (e) => {
    ...
    if (input.endsWith(' ')) {
      ...
      if (found) {
        ...
        setMissed((m) => m + 1); // ✅ Add 1 to missed if wrong word typed
