...
const Game = () => {
  ...
  const bgMusic = useRef(new Audio(bgMusicFile));
   // Use a ref to store missed count without triggering re-renders
  ...
  useEffect(() => {
    const falling = setInterval(() => {
      setWords((prevWords) => {
        ...
        // If there are missed words, increment the counter for this interval
        if (missedWords.length > 0) {
          missedCountInInterval.current += missedWords.length; // Count the missed words
        }
       ...
       // After the interval ends, update the missed count if needed
       const updateMissed = setInterval(() => {
        if (missedCountInInterval.current > 0) {
          setMissed((prev) => prev + missedCountInInterval.current);
          missedCountInInterval.current = 0; // Reset the count for next interval
        }
      }, 100);
      return () =>{
        clearInterval(falling);
        clearInterval(updateMissed);// Clean up the second interval
      };
