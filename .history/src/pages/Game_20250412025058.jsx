...
const Game = () => {
  ...
  const hasInitialized = useRef(false); // Add this line above useEffect
    ...
    fallingIntervalRef.current = setInterval(() => {
      ...
      setWords((prevWords) => {
        ...
        for (const word of prevWords) {
          const nextTop = word.top + 1;
          if (nextTop >= 90) {
            missedNow++; // count once and remove
          } else {
            newWords.push({ ...word, top: nextTop });
          }
        }
      console.log("active value outside the if statement ",active)
        if (missedNow > 0 && active===true) {
          setMissed((prev) => prev + missedNow);
          active=false;
          console.log("Missed:", missedNow);
          console.log("active value inside the if statement",active)
        }
  ...
  }, []); // DO NOT put any dependencies here
