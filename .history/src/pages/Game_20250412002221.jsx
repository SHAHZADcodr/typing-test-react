...
const Game = () => {
  ...
  const hasInitialized = useRef(false); // prevents interval on unmount
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const falling = setInterval(() => {
        setWords((prevWords) => {
          ...
          if (missedWords.length > 0) {
            setMissed((prevMissed) => prevMissed + missedWords.length);
      ...
      return () => clearInterval(falling); // clear interval on unmount
    }, []);
  
    return (
      <div>
        <h1>Missed Words: {missed}</h1>
        {/* Render words here */}
      </div>
    );
  }
