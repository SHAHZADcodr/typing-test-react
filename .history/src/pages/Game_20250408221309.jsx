...
const Game = () => {
  ...
  const handleTyping = (e) => {
    ...
    const found = words.find((w) => w.text === input.trim());
    if (found) {
      pop.play();
      setScore((s) => s + 1);
      ...
      const wrong = words.find((w) => w.text !== input.trim());
      if (wrong) {
        highlightAndRemove(wrong.id, 'red');
      }
      setTyped('');
    }
  };
  ...
  const endGame = () => {
    ...
    navigate('/gameover', {
      state: { score, missed, accuracy, wpm, total }
    });
  };
