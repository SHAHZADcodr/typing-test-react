...
const Game = () => {
  ...
  const gameEndedRef = useRef(false); // ✅ track latest gameEnded in timer
  ...
  useEffect(() => {
    const falling = setInterval(() => {
      setWords((prevWords) => {
        ...
        if (missedCount > 0) {
          setMissed((m) => m + missedCount); // ✅ count correctly
        }
        return filtered;
      });
    }, 100);
  ...
  const handleTyping = (e) => {
    ...
    if (input.endsWith(' ')) {
      ...
      if (found) {
        ...
        setMissed((m) => m + 1); // ✅ count incorrect input
  ...
  const endGame = () => {
    if (gameEndedRef.current) return; // ✅ stop duplicate calls
    setGameEnded(true);
