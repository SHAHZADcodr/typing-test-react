  ...
  useEffect(() => {
    const falling = setInterval(() => {
      setWords((prevWords) => {
        ...
        // Only accumulate missed words if any missed words exist
        if (missedWords.length > 0) {
          missedCountRef.current += missedWords.length; // Accumulate missed words
          console.log("Missed:", missedWords);
          console.log("Missed Count Before Update:", missed);
        }

        // Filter out words that have passed the bottom of the screen
        return updated.filter((word) => word.top < 90);
      });
    }, 100);

    // After each interval, update the missed count once
    const updateMissed = setInterval(() => {
      if (missedCountRef.current > 0) {
        setMissed((prevMissed) => prevMissed + missedCountRef.current);
        missedCountRef.current = 0; // Reset the missed count for the next interval
      }
    }, 100);

    return () => {
      clearInterval(falling);
      clearInterval(updateMissed);
    };
