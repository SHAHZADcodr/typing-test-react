  ...
  useEffect(() => {
    const falling = setInterval(() => {
      setWords((prevWords) => {
        ...
        if (missedWords.length > 0 && missedRef.current !== missed) {
          missedRef.current = missedRef.current + 1; // Increment missed count in ref
          setMissed(missedRef.current); // Update missed state
        }
