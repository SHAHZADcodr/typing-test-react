    ...
    fallingIntervalRef.current = setInterval(() => {
      setWords((prevWords) => {
        ...
        for (const word of prevWords) {
          const newTop = word.top + 1;
  
          if (newTop >= 90) {
            missedWords.push(word); // This one is missed
          } else {
            updatedWords.push({ ...word, top: newTop }); // Still falling
          }
        }
  
        // Count exactly how many were missed (once!)
        if (missedWords.length > 0) {
          setMissed((prev) => prev + missedWords.length);
          console.log("Missed:", missedWords);
        }
  
        return updatedWords; // Keep only words still on screen
      });
    }, 100);
